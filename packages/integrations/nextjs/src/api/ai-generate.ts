...
let sdkInstance: EnterpriseSDK | null = null;

async function getSDK(): Promise<EnterpriseSDK> {
  if (!sdkInstance) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const result = await (sdk.ai as any).generate(prompt, {
            ...options,
            stream: true,
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { status: 'unhealthy', error: 'SDK initialization failed' },
      { status: 503 }
    );
  }
}
